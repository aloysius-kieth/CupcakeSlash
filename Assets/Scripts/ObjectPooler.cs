...
public class ObjectPooler : MonoBehaviour 
{
    ...
    public GameObject GetPooledObjectByType(string type)
    {
        ...
        for (int i = 0; i < pooledObjects.Count; i++)
        {
            if (!pooledObjects[i].activeInHierarchy /*&& pooledObjects[i].GetComponent<IInteractable>()._type.ToString() != null*/)
            {
                if (pooledObjects[i].GetComponent<Interactable>()._type.ToString() == type)
                {
                    return pooledObjects[i];
                }
            }
        ...
        foreach (ObjectPoolItem item in itemsToPool)
        {
            if (item.objectToPool.GetComponent<Interactable>()._type.ToString() == type)
            {
                if (item.Expandable)
                {
                    return CreatePooledObject(item);
    ...
    public List<Interactable> interactablesList;
    GameObject CreatePooledObject(ObjectPoolItem item)
    {
        ...
        {
            interactablesList.Add(obj.GetComponent<Interactable>());
        }
